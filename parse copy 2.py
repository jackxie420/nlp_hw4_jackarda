...
class EarleyChart:
    ...
    def _predict(self, nonterminal: str, position: int) -> None:
        ...
        for rule in self.grammar.expansions(nonterminal):
            new_item = TableEntry(item = Item(rule, dot_position=0, start_position=position), weight = rule.weight, back_pointers=[])
    ...
    def _scan(self, entry: TableEntry, position: int) -> None:
        ...
        if position < len(self.tokens) and self.tokens[position] == entry.next_symbol():
            new_item = entry.with_dot_advanced(0, new_back_pointer=entry)
...
class TableEntry:
    ...
    def __init__(self, item, weight, back_pointers):
        self.item = item
        self.weight = weight
    ...
    def with_dot_advanced(self, added_weight, new_back_pointer) -> TableEntry:
        return TableEntry(item = self.item.with_dot_advanced(),weight = self.weight + added_weight, back_pointers=self.back_pointers+[new_back_pointer])
    
    def __repr__(self) -> str:
        return self.item.__repr__()
    
    def next_next_symbol(self) -> Optional[str]:
        """What's the next, unprocessed symbol (terminal, non-terminal, or None) in this partially matched rule?"""
        assert 0 <= self.dot_position+1 <= len(self.rule.rhs)
        if self.dot_position+1 == len(self.rule.rhs):
            return None
        else:
